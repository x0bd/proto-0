...
import { Mic, SquareSquare, Volume2, VolumeX, Keyboard, MoreHorizontal } from "lucide-react"
    ...
    <div className={cn("relative z-50 flex items-center gap-2", className)}>
      ...
      <AnimatePresence mode="popLayout">
        {state !== "idle" && (
          <motion.div
            ...
            style={{
              backgroundColor: isError ? 'var(--color-destructive)' : 'var(--background)',
          ...
          >
            ...
            {isSpeaking && (
              <>
                <Volume2 className="size-3.5" style={{ color: accentColor }} />
                <span className="text-xs font-medium tracking-wide">Speaking...</span>
              </>
            )}

            {isError && (
              <span className="text-xs font-medium tracking-wide">Connection Error</span>
            )}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        <button
          ...
          style={
            isListening
            ? { backgroundColor: accentColor, color: "#fff", boxShadow: `0 0 20px ${accentColor}80` }
        ...
        {/* Right Toggle (Text Mode / Settings) */}
        <button
