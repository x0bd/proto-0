...
function useScramble(
	text: string,
	{ autoOnMount = false, speed = 26, revealRate = 0.4 } = {},
) {
...
function Scramble({
	...
	const { out, run } = useScramble(text, { autoOnMount });
	return (
...
/* ─── Feature row — hover decode + lime tick ─────────────────────── */
function FeatureRow({ n, title, desc }: { n: string; title: string; desc: string }) {
	const { out, run } = useScramble(title);
	return (
		<div
			onMouseEnter={run}
			className="group flex items-center gap-4 border-t border-[rgba(235,235,235,0.14)] py-2.5 transition-colors last:border-b hover:border-[rgba(235,235,235,0.34)]"
		>
			<span className="font-mono text-[11px] tracking-[0.06em] text-[rgba(235,235,235,0.4)] transition-colors group-hover:text-[rgba(235,235,235,0.7)]">
				[{n}]
			</span>
			<span className="font-mono text-[11px] uppercase tracking-[0.08em] text-[var(--bg)]">
				{out}
			</span>
			<span className="ml-auto hidden font-mono text-[11px] text-[rgba(235,235,235,0.4)] transition-colors group-hover:text-[rgba(235,235,235,0.7)] sm:inline">
				{desc}
			</span>
			{/* lime tick — transient accent geometry on the band */}
			<span className="size-[6px] shrink-0 bg-[var(--accent)] opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
		</div>
	);
}
...
const NAV = ["COMPANION", "MEMORY", "KEYS"] as const;
...
export default function LandingPage() {
	...
	return (
		<main className="min-h-dvh bg-background p-0 text-foreground sm:h-dvh sm:overflow-hidden sm:p-3">
			{/* ─── Instrument window ───────────────────────────────── */}
			<div className="flex h-full flex-col sm:border sm:border-[var(--hair-2)]">
				...
				<header className="flex h-12 shrink-0 items-center justify-between border-b border-[var(--hair)] px-5 sm:px-7">
					...
					<nav className="hidden items-center gap-8 sm:flex">
						{NAV.map((l) => (
							<Scramble
								key={l}
								text={l}
								className="label cursor-default select-none transition-colors hover:text-[var(--fg)]"
							/>
						))}
					</nav>
					...
					<div className="flex items-center gap-4">
						<div className="hidden items-center gap-2 sm:flex">
							<span className="size-[7px] bg-[var(--accent)] lime-pulse" />
							<span className="label">LOCAL · BYOK</span>
						</div>
						<span className="hidden h-3 w-px bg-[var(--hair-2)] sm:block" />
						<Clock />
						<span className="hidden h-3 w-px bg-[var(--hair-2)] sm:block" />
						{LINKS.map((l) => (
							<a
				...
				<div className="grid flex-1 grid-cols-1 sm:min-h-0 sm:grid-cols-[45fr_55fr]">
					...
					<section className="dot-grid-subtle relative flex h-[40vh] flex-col overflow-hidden border-b border-[var(--hair)] sm:h-auto sm:border-b-0">
					...
					>
						...
						<div
							...
							style={{
								backgroundImage:
									"radial-gradient(circle, var(--bg) 1px, transparent 1px)",
						...
						<div className="relative z-10 flex items-center justify-between">
							...
							<span className="label hidden sm:inline">[ READY ]</span>
						</div>
						...
						<div className="relative z-10 flex flex-col gap-6 sm:gap-8">
							...
							>
								<Scramble text={"A SMALL\nAI COMPANION"} autoOnMount />
							</h1>

							<p className="max-w-[44ch] font-mono text-[12px] leading-relaxed text-[rgba(235,235,235,0.55)]">
								A local-first AI companion for voice, memory and mood. Bring
								your own keys — nothing leaves the device.
							</p>
							...
							<div className="flex items-center gap-5">
								<Link
									href="/companion"
									className="flex h-12 items-center justify-center px-11 transition-colors hover:bg-[var(--accent-hover)]"
						...
						{/* pagination / sig */}
						<div className="relative z-10 hidden items-end justify-between sm:flex">
							<span
