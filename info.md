...
The avatar reacts in real time to user input (text or voice) via the AI SDK, producing a **dreamlike, vaporous presence**.
...
1. **User Input**
    ...
    - Text via input box.
    - Optional voice input (capture via Web Audio API / TTS/STT).

2. **AI SDK Processing**

    - Use Vercel AI SDK’s unified API (e.g., `generateObject`, `streamText`, `generateText`, plus TTS/STT) to interpret input. :contentReference[oaicite:3]{index=3}
    - The SDK returns a **JSON‑emotion vector**:
